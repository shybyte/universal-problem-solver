...
impl Debug for PuzzleState {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let s = join(self.playfield.iter().map(|row| join(row, "")), "\n");
        write!(f, "\n{}", s)
    }
}
    ...
    fn is_action_legal(&self, state: &PuzzleState, action: &PuzzleAction) -> bool {
        ...
        for (row_index, row) in puzzle_piece.matrix.iter().enumerate() {
            for (col_index, cell) in row.iter().enumerate() {
                if *cell > 0 && state.playfield[action.row + row_index][action.column + col_index] > 0 {
                    return false;
                }
            }
        }
...
impl Problem<PuzzleState, PuzzleAction> for PuzzleProblem {
    fn get_initial_state(&self) -> PuzzleState {
        PuzzleState {
            piece_count: 0,
            playfield: vec![vec![0; self.cols]; self.rows],
        }
    }
        ...
        for row in 0..self.rows {
            for col in 0..self.cols {
                for rotation in 0..self.pieces_rotations[state.piece_count as usize].len() {
                    let puzzle_piece_matrix = &self.pieces_rotations[state.piece_count as usize][rotation].matrix;
                    if row + puzzle_piece_matrix.len() <= self.rows && col + puzzle_piece_matrix[0].len() <= self.cols {
                        let action = PuzzleAction {
    ...
    fn apply_action(&self, state: &PuzzleState, action: &PuzzleAction) -> PuzzleState {
        ...
        for (row_index, row) in puzzle_piece.matrix.iter().enumerate() {
            for (col_index, cell) in row.iter().enumerate() {
                if *cell > 0 {
                    next_state.playfield[action.row + row_index][action.column + col_index] = action.piece_index as i8 + 1;
                }
            }
        }
    ...
    fn is_success(&self, state: &PuzzleState) -> bool {
        state.playfield.iter().all(|row| row.iter().all(|cell| *cell > 0))
    }
}
